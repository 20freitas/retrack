...
export async function POST(req: Request) {
  ...
  try {
    console.log('[sales/upsert] body=', JSON.stringify(body));
    console.log('[sales/upsert] ownerId=', ownerId);
    console.log('[sales/upsert] payloadKeys=', Object.keys(payload || {}));
  } catch (e) {}
  ...
  try {
    if (id) {
      ...
      if (ownerId) {
        const { data: existing, error: existsErr } = await supabase.from("sales").select("owner_id").eq("id", id).single();
        ...
        if (!existing || existing.owner_id !== ownerId) return new Response(JSON.stringify({ ok: false, error: "Not authorized to update this sale" }), { status: 403, headers: { "Content-Type": "application/json" } });
      }

      const { data, error } = await supabase.from("sales").update(payload).eq("id", id).select();
      try { console.log('[sales/upsert] update payload keys=', Object.keys(payload || {})); } catch (e) {}
      ...
      try { console.log('[sales/upsert] insert payload keys=', Object.keys(payload || {})); } catch (e) {}
