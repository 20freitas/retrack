  ...
  const segments = items
    ...
    .map((it, idx) => {
      ...
      return {
        ...
        color:
          it.color ??
          ["#06b6d4", "#7c3aed", "#10b981", "#f97316", "#ef4444"][idx % 5],
        label: it.label,
        value,
      };
    });
  ...
  const legendRight = (
    <div className="space-y-2 ml-4">
      {items.map((it, idx) => (
        ...
        >
          <div
            style={{ width: 12, height: 12, background: it.color ?? "#888" }}
  ...
  const legendBottom = (
    <div className="mt-4 w-full flex flex-col items-center gap-2">
      {items.map((it, idx) => (
        <div
          key={`${it.label}-${idx}`}
          className="flex items-center gap-3 w-full max-w-md px-2"
        >
          <div
            style={{ width: 12, height: 12, background: it.color ?? "#888" }}
            className="rounded-sm flex-shrink-0"
              ...
              {[
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg transition-all ${
                    color === presetColor
                      ? "ring-2 ring-white ring-offset-2 ring-offset-[#071018] scale-110"
                      : "hover:scale-105"
                  }`}
        ...
        <div className="mt-4 flex gap-3">
          <Button
            onClick={() => {
              ...
              if (!label || v <= 0) return;
              onAdd({ label, value: v, color: color || undefined });
              onClose();
            }}
          >
            Add
...
function ManageVariablesModal({
  ...
  onEdit: (
    index: number,
    updated: { label: string; value: number; color?: string }
  ) => void;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {[1, 2].map((i) => (
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <div className="w-60 h-60 rounded-full bg-white/5"></div>
              <div className="mt-4 space-y-2 w-full">
                {[1, 2, 3].map((j) => (
                  <div key={j} className="h-8 bg-white/5 rounded"></div>
                ))}
              </div>
            </div>
          </div>
...
export default function FinancePage() {
  ...
  const [investVars, setInvestVars] = useState<
    { label: string; value: number; color?: string }[]
  >([]);
  const [salesVars, setSalesVars] = useState<
    { label: string; value: number; color?: string }[]
  >([]);
  const [addModalOpenFor, setAddModalOpenFor] = useState<
    null | "invest" | "sales"
  >(null);
  const [manageModalOpenFor, setManageModalOpenFor] = useState<
    null | "invest" | "sales"
  >(null);
    ...
    async function fetchSales() {
      ...
      try {
        const res = await fetch(
          `/api/sales/list?userId=${encodeURIComponent(userId)}`
        );
  ...
  function filterByRange<T extends { date?: string; sale_date?: string }>(
    arr: T[]
  ) {
  ...
  const filteredSales = useMemo(
    () => filterByRange(sales) as Sale[],
    [sales, filterRange]
  );
  const filteredExpenses = useMemo(
    () => filterByRange(expenses) as Expense[],
    [expenses, filterRange]
  );

  const metrics = useMemo(() => {
    const totalSales = filteredSales.reduce(
      (s, x) => s + (x.sale_price || 0),
      0
    );
    const totalInvested = filteredSales.reduce(
      (s, x) => s + (x.purchase_price || 0),
      0
    );
    const totalProfit = filteredSales.reduce((s, x) => s + (x.profit || 0), 0);
    const itemsSold = filteredSales.length;
    const avgMargin = itemsSold
      ? filteredSales.reduce((s, x) => s + (x.margin || 0), 0) / itemsSold
      : 0;
  ...
  const donutItems = useMemo(() => {
    ...
    const catTotals = expenseCats.map((c) => ({
      label: c,
  ...
  function exportReportCSV(asExcel = false) {
    ...
    const filename = asExcel
      ? `retrack-financial-${filterRange}.xls`
      : `retrack-financial-${filterRange}.csv`;
    ...
    const html = `
      ...
      <body>
        ...
        ${filteredSales
          .map(
            (s) =>
              `<tr><td>${s.sale_date}</td><td>${s.sale_price}</td><td>${
                s.purchase_price ?? 0
              }</td><td>${s.profit ?? 0}</td></tr>`
          )
          .join("")}
        </tbody></table>
        <h2>Expenses</h2>
        ...
        ${filteredExpenses
          .map(
            (e) =>
              `<tr><td>${e.date}</td><td>${e.category}</td><td>${
                e.amount
              }</td><td>${e.note || ""}</td></tr>`
          )
          .join("")}
  ...
  const handleEditInvestVar = (
    ...
    if (index >= 4) {
      ...
      setInvestVars((s) =>
        s.map((item, i) => (i === customIndex ? updated : item))
      );
    }
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <select
          ...
          className="rounded-lg bg-white/10 border border-white/20 px-4 py-2 text-sm text-white font-medium hover:bg-white/15 transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-gray-500"
        >
          <option value="day" className="bg-gray-900 text-white">
            Last day
          </option>
          <option value="week" className="bg-gray-900 text-white">
            Last week
          </option>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setManageModalOpenFor("invest")}
                title="Manage variables"
                className="text-white/80 bg-white/4 hover:bg-white/6 rounded-full w-7 h-7 flex items-center justify-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="14"
              ...
              <button
                onClick={() => setAddModalOpenFor("invest")}
                title="Add variable"
                className="text-white/80 bg-white/4 hover:bg-white/6 rounded-full w-7 h-7 flex items-center justify-center"
              >
                +
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setManageModalOpenFor("sales")}
                title="Manage variables"
                className="text-white/80 bg-white/4 hover:bg-white/6 rounded-full w-7 h-7 flex items-center justify-center"
              >
                <svg
              ...
              <button
                onClick={() => setAddModalOpenFor("sales")}
                title="Add variable"
                className="text-white/80 bg-white/4 hover:bg-white/6 rounded-full w-7 h-7 flex items-center justify-center"
              >
                +
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mt-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <Button variant="ghost" onClick={() => exportReportCSV(false)}>
            Export CSV
          </Button>
          <Button variant="ghost" onClick={() => exportReportCSV(true)}>
            Export Excel
          </Button>
          <Button variant="primary" onClick={exportReportPDF}>
            Export PDF
          </Button>
        </div>
      </div>
